...
def Leaderboard(what_to_do): # Se definen los 4 jugadores con el puntaje mas alto y se imprimen
                ...
                for vkey in leaderboard.keys():
                    if leaderboard[vkey]['NameCountry'] != '':
                        ...
                        if rknt == 1:
                            ...
                            sc3.write(
                                f"🥈 | {leaderboard[vkey]['NameCountry']}: :red[{leaderboard[vkey]['HighestScore']}]")
                        elif rknt == 4: sc4.write(
                                f"🥈 | {leaderboard[vkey]['NameCountry']}: :red[{leaderboard[vkey]['HighestScore']}]")


def InitialPage(): # Se inicializa la pantalla inicial, se definen instrucciones de juego
    with st.sidebar:
...
def ReadPictureFile(wch_fl): # Abre el archivo de la foto
    try:
        pxfl = f"{vpth}{wch_fl}"
...
def PressedCheck(vcell): # Se verifica si un boton este presionado
    if mystate.plyrbtns[vcell]['isPressed'] == False:
        mystate.plyrbtns[vcell]['isPressed'] = True
        mystate.expired_cells.append(vcell)

        if mystate.plyrbtns[vcell]['eMoji'] == mystate.sidebar_emoji: # Cuantos puntos gana segun la dificultad
...
def ResetBoard(): # Generar un nuevo tablero
...
def PreNewGame(): # Se cargan los emojis y se escogen de manera de aleatoria
    total_cells_per_row_or_col = mystate.GameDetails[2]
    ...
    random.seed()
    if mystate.GameDetails[0] == 'Easy': # Dependiedo de la dificultad, se definen la cantidad de categorias que pueden salir
...
def ScoreEmoji(): # Se asigna un emoji en base al puntaje final obtenido
...
def NewGame(): # Se inicializa un juego nuevo
    ResetBoard()
    total_cells_per_row_or_col = mystate.GameDetails[2]
    ...
    with st.sidebar:
        ...
        st.markdown(horizontal_bar, True)
        if st.button(f"🔙 Return to Main Page", use_container_width=True): # Boton para volver a la paina de inicio luego de acabar el juego
...
def Main():
    ...
    with st.sidebar:
        mystate.GameDetails[0] = st.radio('Difficulty Level:', options=('Easy', 'Medium', 'Hard'), index=1,
                                          horizontal=True, )
        mystate.GameDetails[3] = st.text_input("Player Name, Country", placeholder='Esteban Munoz, Colombia',
                                               help='Optional input only for Leaderboard')

        if st.button(f"🕹️ New Game", use_container_width=True): # Boton para crear un juego nuevo
